...
export default function SellerPackagePage() {
  ...
  return <main className="seller-shell seller-package-page">
    ...
    <section className="seller-content"><h2>박스 크기 선택</h2><div className="package-grid">{merchantPackageTypes.map((item) => <article className="package-card" key={item.id}><div className={`box-illustration box-${item.label.toLowerCase()}`} aria-hidden="true"><span>📦</span></div><strong>택배 박스<br />{item.label}</strong><div className="quantity-stepper"><button type="button" aria-label={`${item.label} 수량 감소`} onClick={() => setQuantity(item.id, form.quantities[item.id] - 1)}>−</button><input aria-label={`택배 박스 ${item.label} 수량`} inputMode="numeric" value={form.quantities[item.id]} onChange={(event) => setQuantity(item.id, normalizeQuantity(event.target.value))} /><button type="button" aria-label={`${item.label} 수량 증가`} onClick={() => setQuantity(item.id, form.quantities[item.id] + 1)}>+</button></div></article>)}</div><label className="category-select"><select aria-label="상품 카테고리" value={form.productCategory} onChange={(event) => setForm({ ...form, productCategory: event.target.value })}><option value="">상품 카테고리 선택</option><option>빵류</option><option>과자류</option><option>과일류</option></select></label><label className="icebox-check"><input type="checkbox" checked={form.iceBoxPackaging} onChange={(event) => setForm({ ...form, iceBoxPackaging: event.target.checked })} /><span>아이스박스 포장</span></label>{error && <p className="seller-validation" role="alert">{error}</p>}</section>
