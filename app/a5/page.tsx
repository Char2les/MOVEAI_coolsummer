...
export default function A5Page() {
  const router = useRouter(); const { merchants } = useDataCatalog(); const [query, setQuery] = useState(""); const [filter, setFilter] = useState<"all" | "partner">("all"); useEffect(() => { const params = new URLSearchParams(window.location.search); setQuery(params.get("q") ?? ""); setFilter(params.get("filter") === "partner" ? "partner" : "all"); }, []); const reachableIds = useMemo(() => new Set(getDemoReachability().filter((result) => result.reachable).map((result) => result.merchantId)), []); const items = useMemo(() => filterMerchants(query, filter, merchants).filter((merchant) => reachableIds.has(merchant.id)), [query, filter, merchants, reachableIds]); const sync = `?q=${encodeURIComponent(query)}&filter=${filter}`;
